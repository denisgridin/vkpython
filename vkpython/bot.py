    ...
    if longPoll['updates'] and len(longPoll['updates']) != 0:
        for update in longPoll['updates']:
            if update['type'] == 'message_new':
                ...
                afile = post(api.docs.getMessagesUploadServer(type = 'audio_message', peer_id = update['object']['user_id'])['upload_url'], files = {'file': open('audio.mp3', 'rb')}).json()['file']
                ...
                api.messages.send(peer_id = update['object']['user_id'], message = 'Привет, %s &#128521;'%name, attachment = 'doc%s_%s,photo%s_%s'%(doc['owner_id'], doc['id'], doc['owner_id'], photo['id']))
